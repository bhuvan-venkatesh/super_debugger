...
*/
	...
	do {
      ...
      return 1;
		...
		if ( strlen(str) ) {
			//file_len += strlen( str );
			file_contents += str;
			//file_contents = ( char* )realloc( file_contents, file_len );
			//strncpy( file_contents, str, strlen(str) );
  ...
  if (WIFEXITED(status) && WEXITSTATUS(status) == 1) 
	...
	size_t len;
	char** objectDump = strsplit( &file_contents[0u], "\n", &len );
	...
	while ( incre < len && strstr(objectDump[incre++], "<main>:" ) == NULL );
	...
	while ( 1 ) {
		if ( objectDump[incre][0] == ' ' ) {
			...
			int linelen = strlen( objectDump[incre] );
			int numstart = 0;
			
			for ( int i = linelen - 1; i; --i ) {
				if ( objectDump[incre][i] == ':' ) {
					numstart = i;
					break;
				}
			}
			
			if ( numstart == linelen - 1 )
				break;

			int linenum = atoi( &objectDump[incre][numstart + 1] );
