...
// *** Run with: gcc -g -std=c99 -w testCamel_sol.c -o testCamel_sol ***
...
void parseStr(const char* input_str, char** ret, int count, int idx) {
	...
	ret[idx] = (char*)malloc(count - spaces + 1);
	char* begin = ret[idx];
...
char **camel_caser(const char *input_str) {
	...
	char** output_s = (char*)malloc((numPunct + 1) * sizeof(char*));		// 1 more for NULL at the end
